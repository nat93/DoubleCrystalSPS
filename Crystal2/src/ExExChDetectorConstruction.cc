...
fWorldLogic(0),fXtalLogic(0){
    ...
    // 2018 //
    //====================================================================//
    fXtalAngle = G4ThreeVector((535.513-10.0)*1e-6,0.,0.); // -dTheta --> VR
    fXtalSize = G4ThreeVector(4. * CLHEP::millimeter,
                              50. * CLHEP::millimeter,
                              6. * CLHEP::millimeter);
    fXtalCurvatureRadius = G4ThreeVector(30. * CLHEP::meter,
                                         0. * CLHEP::meter,
                                         0. * CLHEP::meter);
    //====================================================================//
    //====================================================================//
...
G4VPhysicalVolume* ExExChDetectorConstruction::Construct(){
    ...
    G4LogicalVolume* phantom_log_2 = new G4LogicalVolume(phantom_box,fWorldMaterial,"phantom_2");
    new G4PVPlacement(0,G4ThreeVector(0,0, -3.25 * CLHEP::millimeter-3.5*CLHEP::millimeter),phantom_log_0,"phantom_0",fWorldLogic,false,0);
    new G4PVPlacement(0,G4ThreeVector(0,0, -3.25 * CLHEP::millimeter),phantom_log_1,"phantom_1",fWorldLogic,false,0);
    new G4PVPlacement(0,G4ThreeVector(0,0,  3.25 * CLHEP::millimeter),phantom_log_2,"phantom_2",fWorldLogic,false,0);

    phantom_log_0->SetVisAttributes(G4VisAttributes(G4Color::Gray()));
    phantom_log_1->SetVisAttributes(G4VisAttributes(G4Color::Gray()));
    phantom_log_2->SetVisAttributes(G4VisAttributes(G4Color::Gray()));

    // TARGET
    G4Material* TUNGSTEN = G4NistManager::Instance()->FindOrBuildMaterial("G4_W");
    G4Box* target_box = new G4Box("W_target", 5.0*CLHEP::millimeter/2, 30.0*CLHEP::millimeter/2, 3.0*CLHEP::millimeter/2);
    G4LogicalVolume* target_log = new G4LogicalVolume(target_box, TUNGSTEN, "W_target");
//    new G4PVPlacement(0,G4ThreeVector(
//                          -22.7284*CLHEP::millimeter-0.5*CLHEP::millimeter,
//                          0.0*CLHEP::millimeter,
//                          -5.0*CLHEP::millimeter),target_log, "W_target", fWorldLogic, false, 0);

    target_log->SetVisAttributes(G4VisAttributes(G4Color::Brown()));
    ...
    if(fXtalCurvatureRadius.x() != 0.){
        ...
        fXtalPhysical =
            new G4PVPlacement(vRotationMatrix,
                              G4ThreeVector(-fXtalCurvatureRadius.x()-0.0227284 * CLHEP::meter,0.,-fXtalSize.z()/2.),
                              fXtalLogic,"Target",
                              fWorldLogic,
                              false,
                              0);
